...
## 1.初始化购物车  
### 触发：<br>
>> 1)如果用户操作购物车（操作购物车，查看购物车），但是本地没有购物车。<br>
### 操作：<br>
>> 1)如果用户没有登陆，看线下是否有购物车，如果线下有购物车就直接获取线下购物车；如果线下没有购物车就创建一个购物车。<br>
>> 2)如果用户登陆：<br>
>> ①.按照接口文档3.1把参数按照规则进行传输。<br>
>> ②.发现前台没有提供购物车信息，默认线下没有购物车，如果后台查询到相关的数据就返回给前台，如果没有，就返回一个null。<br>
...
>> 2)如果用户登陆状态，使用接口3.3，按照4.3进行接口参数验证，如果失败就返回错误信息。成功之后就把card_item表中card_id对应数据滞空。
## 4.清空购物车
### 触发：<br>
>> 1)用户点击清空购物车。
### 操作：<br>
>> 1)如果用户未登陆，直接操作本地购物车，清空本地购物车。
...
# 四. 验证
## 1. 更新购物车验证
>> 1) 先按照接口表中的字段获取各个参数，根据表中的“是否必填”和“类型(长度)”对字段进行验证，如果错误返回错误信息。<br>
>> 2) 如果包含cartId就验证该购物车是否存在，不存在就返回没有购物车的错误提示。
>> 3) 购物车中不能出现两类产品，如果出现就返回提示错误信息。<br>
>> 4) 对产品的剩余量进行验证， 如果错误就返回现在的剩余量。<br>
>> 5) 按照产品的规则（最少购买份数|最多购买份数），返回错误信息提示最小购买额|最大购买额。<br>

## 2. 修改购物车产品数量验证
>> 1) 先按照接口表中的字段获取各个参数，根据表中的“是否必填”和“类型(长度)”对字段进行验证，如果错误返回错误信息。<br>
>> 2) 如果包含cartId就验证该购物车是否存在，不存在就返回没有购物车的错误提示
>> 3) 如果判断到用户是登陆状态，购物车中不能出现两类产品，如果出现就返回提示错误信息<br>
>> 4) 对产品的剩余量进行验证， 如果错误就返回现在的剩余量。<br>
>> 5) 按照产品的规则（最少购买份数/最多购买份数），返回错误信息提示最小购买额/最大购买额。<br>

## 3. 删除购物车中产品
>> 1) 先按照接口表中的字段获取各个参数，根据表中的“是否必填”和“类型(长度)”对字段进行验证，如果错误返回错误信息。<br>
>> 2) 验证该购物车是否存在，不存在就返回没有购物车的错误提示。
>> 3) 获取当前登陆用户，对用户权限进行验证，如果权限不足就返回权限不足的错误提示。

## 4. 清空购物车
>> 1) 先按照接口表中的字段获取各个参数，根据表中的“是否必填”和“类型(长度)”对字段进行验证，如果错误返回错误信息。<br>
>> 2) 就验证该购物车是否存在，不存在就返回没有购物车的错误提示
>> 3) 获取当前登陆用户，对用户权限进行验证。
